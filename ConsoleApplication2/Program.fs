...
open Casanova.StandardLibrary.Core
    ...
    and [<CasanovaEntity>] Alien = {
      Position : Rule<Vector2<m>>
      Life     : Var<float32>
      Sprite   : DrawableSprite
    } with
     static member Position'(self:Alien,dt:float32<s>) = !self.Position + dt * Vector2<m/s>.UnitY * 40.0f

     static member SpritePosition'(self:Alien) = !self.Position * 1.0f<pixel/m>
  ...
  let world0 = 
    {
      Ship = 
        {
          Position = Rule.Create(Vector2<m>(0.0f<m>,450.0f<m>))
          Velocity = Rule.Create(Vector2<m/s>.Zero)
          Sprite   = DrawableSprite.Create(game.default_layer, Vector2<pixel>.One, Vector2<pixel>.One * 100.0f, @"newship")
        }
       
      Alien =
       {
          Position = Rule.Create(Vector2<m>(250.0f<m>,-400.0f<m>))
          Life = Var.Create(1.0f)
          Sprite   = DrawableSprite.Create(game.default_layer, Vector2<pixel>.One, Vector2<pixel>.One * 100.0f, @"alien1")
       }
      Alien1 =
       {
          Position = Rule.Create(Vector2<m>(250.0f<m>,-300.0f<m>))
          Life = Var.Create(1.0f)
          Sprite   = DrawableSprite.Create(game.default_layer, Vector2<pixel>.One, Vector2<pixel>.One * 100.0f, @"alien1")
       }
    }
